 ...
 * @returns 包含三个月数据的对象
 */
export function getMonthLazyLoadData(year: number, month: number): LazyLoadData<MonthData> {
  // 计算前一月
  ...
  return {
    prev: {
      ...
      firstDayOfWeek: getFirstDayOfMonth(prevYear, prevMonth),
    },
    current: {
      year,
      month,
    ...
    next: {
      ...
      firstDayOfWeek: getFirstDayOfMonth(nextYear, nextMonth),
    },
  };
}
